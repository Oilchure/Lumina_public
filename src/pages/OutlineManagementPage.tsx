...
import { EditIcon, TrashIcon, FolderPlusIcon, ChevronDownIcon, ChevronRightIcon } from '../components/icons.tsx';
...
const OutlineManagementPage: React.FC = () => {
  ...
  const renderCategoryOptionsForParentSelect = (currentEditingCategoryId?: string, parentId: string | null = null, depth = 0): React.ReactNode[] => {
    return categories
      .filter(cat => {
        if (cat.id === currentEditingCategoryId) return false;
    ...
    <div className="space-y-8">
      <div>
        <div className="flex justify-between items-center mb-6">
            ...
            <button
                onClick={() => handleOpenCategoryModal(undefined, null)}
                className="flex items-center bg-indigo-500 hover:bg-indigo-600 text-white font-semibold py-2 px-3 sm:px-4 rounded-md shadow transition-colors"
                aria-label="添加新顶级分类"
            >
                <FolderPlusIcon className="w-5 h-5 sm:mr-2" />
                <span className="hidden sm:inline">添加顶级分类</span>
            </button>
        </div>
