...
import { Word, WordDefinition, EbbinghausStage, EBBINGHAUS_REVIEW_OFFSETS_DAYS, PartOfSpeechCh, CHINESE_POS_TERMS, ReadingRecordSourceInfo } from '../types.ts';
import { 
...
import { PlusIcon, ChevronUpIcon, ChevronDownIcon } from '../components/icons.tsx';
import { fetchWordDefinitions } from '../services/dictionaryService.ts';
...
const WordsPage: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingWord) {
      ...
      wordToSave = {
        id: `word-${now}-${Math.random().toString(36).substring(2,9)}`,
        text: currentWordText.trim(),
        definitions: finalDefinitions,
        notes: currentNotes.trim(),
        readingRecordSource: sourceToSave,
  ...
  const handleMarkAsRememberedWord = (word: Word) => {
  ...
  const handleMarkAsForgottenWord = (word: Word) => {
    updateWord({
      ...word,
      reviewStage: EbbinghausStage.LEARNED, 
      lastReviewedAt: Date.now(), 
  ...
  const handleUndoReviewWord = (word: Word) => {
  ...
  const displayedWords = useMemo(() => {
    ...
    // viewMode === 'all'
    if (!searchTerm.trim()){
         return words; // Use the full words list from context if no search term
    }
    return words.filter(word => // Filter the full words list if search term exists
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 p-4 bg-white dark:bg-slate-800 rounded-lg shadow">
        <h1 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-slate-100">
            {viewMode === 'review' ? `词汇复习 (${wordsForReview.length})` : '词汇列表'}
        </h1>
      ...
      {displayedWords.length === 0 && viewMode === 'all' && !searchTerm && words.length === 0 && (
      ...
      {displayedWords.length === 0 && viewMode === 'all' && searchTerm && (
         <div className="text-center py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {displayedWords.map(word => (
          <WordCard 
            ...
            onDelete={handleDeleteWordDirect} 
            onUndoReview={handleUndoReviewWord}
            isReviewMode={viewMode === 'review'}
            onMarkAsRemembered={handleMarkAsRememberedWord}
            onMarkAsForgotten={handleMarkAsForgottenWord}
          />
        ))}
      </div>
